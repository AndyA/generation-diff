...
class BaseReader {
  constructor(bb, key) {
    ...
    this.nextIn = this.section.current?.pos || 0;
  }

  get available() {
    const { nextIn, bb } = this;
  ...
  async nextInput(extraMutator) {
    ...
    const offset = this.nextIn;
    const next = (this.nextIn = this.bb.incrementOffset(offset));
...
class StoreReader extends BaseReader {
  async seek(offset) {
    ...
    const token = this.section.makeToken(
      state => {
        state.pos = offset;
      },
      () => (this.inPos = offset)
