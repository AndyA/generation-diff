...
class ObjectBrigadeBase {
  ...
  async flushBuffer() {
    ...
    if (buffer.length) {
      ...
      // When the block has been flushed and its state updated we
      // trigger any tokens attached to output objects
      await token(null, () =>
        Promise.all(buffer.map(({ token }) => token).filter(Boolean))
      );
    }
  }

  async write(object, token) {
    if (this.buffer.length >= this.opt.chunkSize) await this.flushBuffer();
    this.buffer.push({ object, token });
  }
